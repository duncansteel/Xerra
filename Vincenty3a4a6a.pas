program vincenty3a4a6a(input, output);

{Duncan Steel, 2020 September 16th}
{Output folder changed 2020 October 06}

{Program for solving the Vincenty forward/direct problem}
{This version is for testing the use of equations 3a, 4a & 6a in Vincenty (1975) agaist the full accuracy method}

var
  i,j,k,n : integer;
  pi,piby2,twopi,degrad,raddeg,gmearth,fo16,sma,smb,sma2,smb2,dsm2,e2,oneof,onemf,aob,f,dist,alpha1,alpha2,a2b2b2,
  la1,la2,lo1,lo2,dstep,lab,lob,alphab,laa,loa,alphaa,la346,lo346,alpha346 : real;
  ofl : text;
  c : char;
...
procedure vincenty346a(lat1,lon1,alph1,pstep : real; var lan,lon,alphn : real);

{The version of the direct/forward procedure using Vincenty (1975) equations 3a, 4a & 6a}

var u1,tu1,salph1,bigl,lam,alpha,sigma,sig1,twosigm,ctwosigm,c2twosigm,q1,q2,sqsalph,calph1,d1,d2,d3,d4,
    s1,c1,ssig,csig,su1,cu1,deltsig,salph,csqalph,bigc,biga,bigb,usq,psa,b4,dum,sigmaold : real;
    nn : integer;
...
if (u1 > 999.9) then writeln('u1 error in Vincenty 346a');
salph1:=sin(alph1);
calph1:=cos(alph1);
sig1:=atan2(tu1,calph1);
salph:=cu1 * salph1;
...
{Vincenty's equations 3a & 4a to derive A & B: }
d1:=(5.0 * usq) - 12.0;
d2:=d1 * usq;
d3:=d2 + 64.0;
d4:=(usq/256.0) * d3;
...
deltsig:=1000.0; {large start value}
sigmaold:=1000.0; {large start value}
nn:=0; {Counter}
while ((abs(sigma-sigmaold) > 1.0E-12) and (nn < 11)) do begin
nn:=nn+1;
twosigm:=(2.0*sig1) + sigma;
ctwosigm:=cos(twosigm);
c2twosigm:=sqr(ctwosigm);
...
csig:=cos(sigma);
d1:=bigb * ssig;
d4:=b4 * csig * (dum - 1.0); {for 6a}
deltsig:=d1 * (ctwosigm + d4);    {complete 6a}
sigmaold:=sigma;
sigma:=psa + deltsig;
                                end; {while}
ssig:=sin(sigma);
...
end; {of procedure vincenty346a}
{------------------------------------------------------------------}

begin
assign(ofl,'Vincenty3a4a6a.txt');
rewrite(ofl);
writeln; writeln;
...
writeln(ofl);
dstep:=dist/1000.0; {Each step is 1/1000th of the total distance, in metres}
...
writeln(ofl,i:6,la1:15:6,lo1:15:6,alpha1:15:6,lab:15:9,lob:15:9,alphab:15:9);
vincenty346a(la346,lo346,alpha346,dstep,laa,loa,alphaa);       {for a small step along the path}
writeln(ofl,'**346a',la346:15:6,lo346:15:6,alpha346:15:6,laa:15:9,loa:15:9,alphaa:15:9);
alpha1:=alphab;
la1:=lab;
lo1:=lob;
alpha346:=alphaa;
la346:=laa;
lo346:=loa;
if (abs(lab - laa) > 0.00000001) then writeln('lab,laa difference at i=',i:4);
if (abs(lob - loa) > 0.00000001) then writeln('lob,loa difference at i=',i:4);
if (abs(alphab - alphaa) > 0.00000001) then writeln('alphab,alphaa difference at i=',i:4);
                    end; {i=1 to 1000}
writeln; writeln;
 close(ofl);
