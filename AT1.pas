program AT1(input, output, stdErr);

{Duncan Steel, 2020 October 21}
...
var
  ...
  r,ctarlat,tarlat,tarlon,xt,yt,zt,xs,ys,zs,slat,slat2,clat,sma,smb,gmearth,e2,fss,lon,
  satrad,pi,degrad,raddeg,sma2,smb2,pvr,satlat,satlon,b2oa2,d1,da,db,dc,dd,dx,dy,dz : real;
  ifl,ofl,pfl : text;
  m : array[1..3] of char;
  c : char;
...
clat:=cos(tarlat);
slat2:=sqr(slat);
pvr:=sma / sqrt(1.0 - (e2*slat2));

xt:=pvr*clat*cos(tarlon);
yt:=pvr*clat*sin(tarlon);
zt:=b2oa2 * pvr * slat;
ctarlat:=arctan(b2oa2*slat/clat); {geocentric latitude of the target in radians}
...
writeln; writeln;
writeln(ofl);
writeln(ofl,'Target geocentric latitude and longitude: ',ctarlat*raddeg:12:4,tarlon*raddeg:12:4);
writeln(ofl);
...
d1:=cos(satlat);
xs:=satrad * d1 * cos(satlon) * 1000.0;   {all in metres}
ys:=satrad * d1 * sin(satlon) * 1000.0;
zs:=satrad * b2oa2 * sin(satlat) * 1000.0;
...
writeln(pfl,yy:6,mm:4,day:4,hh:4,mn:4,ss:4,xs/1000.0:15:3,ys/1000.0:15:3,zs/1000.0:15:3);
