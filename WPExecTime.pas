...
var
  ...
  pi,piby2,twopi,degrad,raddeg,sma,smb,sma2,smb2,la1,la2,lo1,lo2,dlat,dlon,theta,d,t1,t2,t3,t4,r,psi,
       ...
       salp0,calp0,loni,lati,sigi,llon,llat,dlonn,lsig,totd,xd,alp,x,y,avt : real;
  t00,t01 : TDateTime;
  ofl,rfl,dfl : text;
  c : char;
...
{
nn:=round(abs(psi*raddeg*100.0));
assign(rfl,'EarthRadii.txt');
reset(rfl);
for m:=0 to nn do readln(rfl,j,y,x);
close(rfl);
r:=x;
}
...
{Using the following increases the execution time by a factor of about 200}
{
nn:=round(abs(psi*raddeg*100.0));
assign(rfl,'EarthRadii.txt');
reset(rfl);
