program HexH3(input, output);

{Duncan Steel, 2020 September 17}
{Output folder changed 2020 October 06}

{Program for working on H3/Hexagon problem from Laszlo}

var
  i,j,n : integer;
  pi,piby2,twopi,degrad,raddeg,gmearth,fo16,sma,smb,sma2,smb2,dsm2,e2,oneof,onemf,aob,f,dist,alpha1,alpha2,a2b2b2,
  la1,la2,lo1,lo2,dstep,lab,lob,alphab,latp1,lonp1,alphap1,distp1,latp2,lonp2,alphap2,distp2,rk,alphay : real;
  ofl : text;
  c : char;
...
procedure vincentydirect(lat1,lon1,alph1,pstep : real; var lan,lon,alphn : real);
...
end; {of procedure vincentydirect}

{------------------------------------------------------------------}

procedure lineapproach(qk,dist,latp1,lonp1,latp2,lonp2 : real;
                  var la1,lo1,alpha1,distp1,distp2 : real);

var sstep,bstep,adiff,x,y,z,lax,lox,alphax,alphad : real;
    m : integer;

begin
m:=1000;
x:=1.0*m; {x will be the maximum number of steps, as a real number}
sstep:=dist/(qk*m); {Each step is 1/(qk*m) of the total distance, in metres}
writeln(ofl,' Step length =',(sstep/1000.0):8:3,' km');
writeln(ofl);
writeln(ofl,' Step      Lat         Lon           AzVtoP1  DistVtoP1      AzVtoP2  DistVtoP2       AzDiff ');
adiff:=0.0; {Start value}
y:=0.0;     {Start value}
z:=(10.0/qk)+0.001;   {Initial limit on azimuth difference away from 180 deg}
while ((abs(adiff - 180.0) > z) and (y < x)) do begin
y:=y + 1.0;      {Maximum of x loops}
distp1:=1.0E6;   {Start values}
distp2:=1.0E6;
bstep:=sstep;
while ((distp1 > 0.6E5) and (distp2 > 0.6E5)) do begin     {Hexagon sides up to one degree = 111 km}
vincentydirect(la1,lo1,alpha1,bstep,lax,lox,alphax);       {for a step bstep along the vessel path}
la1:=lax; {New lat}
lo1:=lox; {New lon}
alpha1:=alphax; {New vessel azimuth at lax,lox = la1,lo1}
vincentyinverse(la1,lo1,latp1,lonp1,alphap1,alphad,distp1); {Renders the azimuth alphap1 from the vessel to point P1}
vincentyinverse(la1,lo1,latp2,lonp2,alphap2,alphad,distp2); {Renders the azimuth alphap2 from the vessel to point P2}
bstep:=5.0E4;    {50 km}
                                                end; {distp > 1.2E5}
adiff:=alphap1 - alphap2;
while (adiff < 0.0) do adiff:=adiff + 360.0;
while (adiff > 360.0) do adiff:=adiff - 360.0;


writeln(ofl,round(y):6,la1:12:6,lo1:12:6,alphap1:14:3,distp1/1000.0:10:3,alphap2:14:3,distp2/1000.0:10:3,adiff:14:3);
                    end; {while adiff & y<x}

if (y > (0.999*x)) then writeln('Too many steps in procedure lineapproach',y:10:3);
if (y > (0.999*x)) then writeln(ofl,'Too many steps in procedure lineapproach',y:10:3);
writeln(ofl);

end; {of procedure lineapproach}

{------------------------------------------------------------------}

begin
assign(ofl,'HexH3.txt');
rewrite(ofl);
writeln; writeln;
...
lo1:=40.5; {These are the overall start and end points: longitude and latitude}
la1:=42.5;  {Point A}
lo2:=30.0;
la2:=40.0;  {Point B}
...
vincentyinverse(la1,lo1,la2,lo2,alpha1,alpha2,dist);
    {This renders the initial azimuth alpha1, and distance dist if wanted}
latp1:=43.3;    {Point P1}
lonp1:=40.0;
latp2:=42.3;    {Point P2}
lonp2:=40.0;
...
dist:=1.0E6;
writeln(ofl);
writeln(ofl,'Start latitude and longitude =',la1:9:3,lo1:9:3,' degrees  ');
writeln(ofl);

rk:=1.0;
lineapproach(rk,dist,latp1,lonp1,latp2,lonp2,la1,lo1,alpha1,distp1,distp2);
rk:=10.0;
lineapproach(rk,dist,latp1,lonp1,latp2,lonp2,la1,lo1,alpha1,distp1,distp2);
rk:=100.0;
lineapproach(rk,dist,latp1,lonp1,latp2,lonp2,la1,lo1,alpha1,distp1,distp2);
rk:=1000.0;
lineapproach(rk,dist,latp1,lonp1,latp2,lonp2,la1,lo1,alpha1,distp1,distp2);
{The above will give the line interception good to about one metre}
{
rk:=10000.0;
lineapproach(rk,dist,latp1,lonp1,latp2,lonp2,la1,lo1,alpha1,distp1,distp2);
}
