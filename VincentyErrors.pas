program vincentyerrors(input, output);

{Duncan Steel, 2020 September 7th}
{Output folder changed 2020 October 06}

{Program for looking into errors in my Vincenty procedures}

var
  i,j,k,n,jjj : integer;
  pi,piby2,twopi,degrad,raddeg,gmearth,fo16,sma,smb,sma2,smb2,dsm2,e2,oneof,onemf,aob,f,dist,alpha1,alpha2,a2b2b2,
  la1,la2,lo1,lo2,dstep,lab,lob,alphab,lax,lox,alphax : real;
  ofl,xfl : text;
  c : char;
...
{
if ((jjj > 190) and (jjj < 200)) then writeln(xfl,jjj:6,(s1/c1):12:6,tu1:12:6,u1*raddeg:14:6);
if ((jjj > 810) and (jjj < 820)) then writeln(xfl,jjj:6,(s1/c1):12:6,tu1:12:6,u1*raddeg:14:6);
}
...
usq:=csqalph*a2b2b2;
{
if ((jjj > 190) and (jjj < 200)) then writeln(xfl,jjj:6,alph1*raddeg:12:6,salph1:12:6,calph1:12:6,sig1*raddeg:12:6);
if ((jjj > 810) and (jjj < 820)) then writeln(xfl,jjj:6,alph1*raddeg:12:6,salph1:12:6,calph1:12:6,sig1*raddeg:12:6);
writeln(xfl,jjj:6,u1*raddeg:18:10,cu1:18:10,sig1*raddeg:18:10,salph*raddeg:18:10,usq:18:10);
writeln(xfl,jjj:6,sig1*raddeg:18:10,u1*raddeg:18:10,cu1:18:10,salph1:18:10,salph:28:20,usq:28:20);
}
...
{writeln(xfl,jjj:6,salph:28:20,usq:28:20,biga:28:20,bigb:28:20,psa:28:20);}

deltsig:=1000.0; {large start value}
nn:=0;
while ((abs(deltsig/sigma) > 0.01) and (nn < 5)) do begin
nn:=nn+1;
...
if (nn>1) then
   writeln(xfl,jjj:6,nn:6,twosigm*raddeg:18:10,ctwosigm:18:10,c2twosigm:18:10,ssig:28:20,csig:28:20);
if (nn>1) then
   writeln(xfl,jjj:6,nn:6,twosigm*raddeg:18:10,ctwosigm:18:10,c2twosigm:18:10,ssig:28:20,csig:28:20);
...
if (nn>1) then
   writeln(xfl,jjj:6,nn:6,d1:28:20,d2:28:20,d3:28:20,d4:28:20,sigma*raddeg:28:20,deltsig*raddeg:28:20,deltsig/sigma:28:20);
if (nn>1) then
   writeln(xfl,jjj:6,nn:6,d1:28:20,d2:28:20,d3:28:20,d4:28:20,sigma*raddeg:28:20,deltsig*raddeg:28:20,deltsig/sigma:28:20);

                                end; {while}
if (nn > 9) then writeln(ofl,jjj:6,nn:6);
ssig:=sin(sigma);
csig:=cos(sigma);
...
begin
assign(ofl,'Vincentytest.txt');
rewrite(ofl);
assign(xfl,'Vincentyerrors.txt');
rewrite(xfl);
writeln; writeln;
...
vincentyinverse(la1,lo1,la2,lo2,alpha1,alpha2,dist);
   {This renders the initial azimuth, alpha1, for use in vincentyforward below}
...
writeln(ofl);
dstep:=dist/1000.0;         {1,000 steps, in metres}
writeln(ofl,'(Vincenty Direct Problem solution)');
writeln(ofl,'With small steps of length ',dstep/1000.0:9:5,' km:');
writeln(ofl,'  Step   Start latitude Start longitude Start azimuth  End latitude   End longitude   End azimuth');

lax:=la1;      {start values}
lox:=lo1;
alphax:=alpha1;
for jjj:=1 to 1000 do begin       {1,000 steps}
vincentyforward(lax,lox,alphax,dstep,lab,lob,alphab);       {for a small step along the path}
writeln(ofl,jjj:6,lax:15:6,lox:15:6,alphax:15:6,lab:15:6,lob:15:6,alphab:15:6);
lax:=lab;
lox:=lob;
alphax:=alphab;
                    end; {i=1 to ...}
