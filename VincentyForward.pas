program vincentyforward(input, output);

{Duncan Steel, 2020 October 05}
...
var
  ...
  la1,la2,lo1,lo2,dstep,lab,lob,alphab : real;
  ofl,cfl,afl : text;
  c : char;
...
begin
assign(ofl,'VincentyForward.txt');
rewrite(ofl);
assign(cfl,'VincentyForward.csv');
rewrite(cfl);
assign(afl,'VincentyForwardAz.csv');
rewrite(afl);
writeln; writeln;
...
writeln(cfl,i:6,' , ',la1:10:6,' , ',lo1:10:6);
la1:=lab;
lo1:=lob;
alpha1:=alphab;
writeln(afl,i+1:6,' , ',alpha1:10:6);
                    end; {i=1 to n}
writeln(cfl,n+1:6,' , ',la1:10:6,' , ',lo1:10:6);
writeln; writeln;
